  ...
  for(i=0; i<Info.length/2; i++)
     {
         ...
         lY  = (Info.FArray[i] >> 16);
         X = Info.length * ((float)lX)/ 32768;
         Y = Info.length * ((float)lY)/ 32768;
         Mag = sqrt(X * X + Y * Y) / Info.length;
		     Info.omega[i]=(float)(1.0f*i*(float)(Info.SampleFreq)/(Info.length/2.0f));
         if(i == 0)            
					 Info.lBufMagArray[i] = (unsigned long)(Mag * 32768);
         else
           Info.lBufMagArray[i] = (unsigned long)(Mag* 65536);
			   Info.Amplitude[i]=(uint16_t)(Info.lBufMagArray[i]*3.3f/4096.0f*2000.0f);
